...
def optimize_portfolio(sd=dt.datetime(2008,1,1), ed=dt.datetime(2009,1,1), \
    ...
    def portfolio_vol(alloc,port_data=prices_norm, sd=dt.datetime(2008,1,1), ed=dt.datetime(2009,1,1), \
...
#   A guess for allocations
    ...
    # Find allocation (in vector form) which minimizes volatility (standard deviation) of daily returns
    allocs = opt.minimize(portfolio_vol,allocs,method="SLSQP",bounds=bnds,constraints=cons).x
    print(allocs)
    alloc_val = (prices_norm)*allocs    
    port_val = alloc_val.sum(1)
    print(port_val)
    ...
    if gen_plot:
        ...
        df_temp = pd.concat([port_val, SPY_norm], keys=['Portfolio', 'SPY'], axis=1)
        plot_data(df_temp)
